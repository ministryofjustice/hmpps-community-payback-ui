...
test.fail('Create an appointment for group session', async ({ page, deliusUser, team, project, personOnProbation }) => {
  await page.goto('/sign-out')
  await expect(page.locator('h1')).toContainText('Sign in')

  const homePage = await signIn(page, deliusUser)
  const groupSessionPage = await searchForASession(page, homePage, team, new Date())
