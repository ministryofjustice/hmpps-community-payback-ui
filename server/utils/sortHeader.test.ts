...
describe('sortHeader', () => {
  ...
  it("should return a header when the current view is not sorted by the header's field", () => {
    expect(sortHeader<SortHeaders>('Some text', 'myField', 'otherField', 'asc', hrefPrefix)).toEqual({
      html: `<a class="moj-sortable-table__button" href="${hrefPrefix}${createQueryString({ sortBy: 'myField' })}">Some text</a>`,
      attributes: {
        'aria-sort': 'none',
  ...
  it('should append a table locator to the href', () => {
    expect(sortHeader<SortHeaders>('Some text', 'myField', 'otherField', 'asc', hrefPrefix, 'my-table')).toEqual({
      html: `<a class="moj-sortable-table__button" href="${hrefPrefix}${createQueryString({ sortBy: 'myField' })}#my-table">Some text</a>`,
      attributes: {
        'aria-sort': 'none',
        'data-cy-sort-field': 'myField',
      },
